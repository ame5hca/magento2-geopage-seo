...
class CategoryPage extends AbstractEntity
{
    const ENTITY_CODE = 'category_page_import';

    const TABLE = 'giftgroup_city_category_page';

    const ENTITY_COLUMN = 'category_id';
    ...
    private function saveAndReplaceEntity()
    {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            foreach ($bunch as $rowNum => $row) {
                ...
                $rowId = $row[static::ENTITY_COLUMN];
                //$rows[] = $rowId;
                ...
                $cityCode = $this->getProcessedValue($row, 'code');
                $columnValues['city_id'] = $this->cityData[$cityCode];
        ...
        if ($entityData) {
            ...
            foreach ($entityData as $entityRows) {
                foreach ($entityRows as $row) {
                    $storeRows = $this->getCategoryPagesForStores($row);
    ...
    {
        $rows = [];
        $categoryPages = $this->getCityPagesToCreateUrlRewrites();
        foreach ($categoryPages as $categoryPage) {
            ...
            $requestPath = sprintf(
                ...
                $this->stateCodeForId[$categoryPage['state_id']], 
                $this->cityCodeForId[$categoryPage['city_id']],
    ...
    {
        return $this->additionalDataProvider->getLastAddedCategoryPages();
    }
    
    private function getCategoryPagesForStores($row)
    {
        $storeSpecificCategoryRows = [];
        ...
        return $storeSpecificCategoryRows;
    }
